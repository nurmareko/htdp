...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |483 IT WORK|) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp")) #f)))
(define SCALE 40)
...
(define (place-queens a-board n)
  (cond
    ...
    [else
     (local (
             (define spots (find-open-spots a-board))
             (define candidate (find-pattern spots n a-board)) ;[List-of [Maybe QP]]
             )
       (cond
         [(boolean? candidate) #false]
         [else candidate]))]))

; [List-of QP] N Board -> [List-of [Maybe QP]]
; ...
  ...
  (cond
    ...
    [else
     (local (
       ...
       (cond
         [(boolean? candidate)
          (find-pattern (rest spots) n a-board)]
         [else candidate]))]))
