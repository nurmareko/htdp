...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |338 339 340 341 342|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require htdp/dir)
...
; if file wtih name, f, is in given Dir, d,
; produce path to such file otherwise #false.

(check-expect (find (make-dir "adir" '() '()) "a.pdf") #false)
(check-expect (find (make-dir "adir" '() `(,image1)) "image1.tiff") (list "adir" "image1.tiff"))
(check-expect (find collection "not.mkv") #false)
(check-expect (find collection "video.mkv")
              (list "Collection" "Movies" "video.mkv"))

(define (find d f)
  (local (; current dir name
          (define cd-name (dir-name d))
          ; is file with name, f, in current Dir files field?
          (define in-dir? (member? f (map file-name (dir-files d))))
          ; [NEList-of Dir] -> [NEList-of String]
          (define (for-dirs ld)
            (local ((define cd (first ld)))
              (if (find? cd f) (find cd f) (for-dirs (rest ld))))))
    (cond
      [(not (find? d f)) #false]
      [else (if in-dir?
                (list cd-name f)
                (cons cd-name (for-dirs (dir-dirs d))))])))
