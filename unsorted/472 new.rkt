...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |472 new|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
; A Node is a Symbol.
...
; (find-path 'A 'G sample-graph) result is
; '(A B E F G) because find-path/list use structural
; recursion, therefore it will try to find a path in
; the first item of the list, and when such path is
; discovered the search ended.
...
(check-expect (test-on-all-nodes a-graph) #true)
