...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |482|) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp")) #f)))
(define QUEENS 8)
; A QP is a structure:
;   (make-posn CI CI)
...
(define (place-queens a-board n)
  (cond
    ...
    [else
     (local (
             (define open-spots (find-open-spots a-board))
             )
       (if (empty? open-spots)
           #false
           (place-queens (add-queen a-board (first open-spots)) (sub1 n))))]))
